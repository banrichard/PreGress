...
def importance_cal(data):
    for cnt,d in enumerate(data):
        ...
        for i in range(len(graph.nodes)):
            ...
            graph.nodes[i]['eigenvector_centrality'] = eigenvector_centrality[i] if type(eigenvector_centrality) == dict else -1
...
def graph_to_file(graph, name, i):
    ...
    with open("/mnt/data/lujie/metacounting_dataset/"+name+"/networkx/"+str(i)+ ".txt","w") as file:
        for node in graph.nodes(data=True):
            file.write("v {} {} {} {} {}\n".format(node[0], node[1]['x'],node[1]['degree_centrality'], node[1]['eigenvector_centrality'], node[1]['betweenness_centrality']))
